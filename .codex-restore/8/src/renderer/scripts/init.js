...
const currentViewPath = (window.location.pathname || '').toLowerCase();
const isProtectedView = !currentViewPath.endsWith('/login.html') && !currentViewPath.endsWith('/register.html');

if (isProtectedView && !localStorage.getItem('sesionActual')) {
...
const syncSessionFromAuthState = (user = {}) => {
    ...
    try {
        localStorage.setItem('sesionActual', JSON.stringify(sessionPayload));
        localStorage.setItem('userName', [sessionPayload.nombre, sessionPayload.apellido].filter(Boolean).join(' ').trim() || sessionPayload.nombre || 'Usuario');
        if (user.lastLoginAt) {
            localStorage.setItem('sesionLastLogin', user.lastLoginAt);
        }
...
const ensureProtectedViewAccess = async () => {
    if (!isProtectedView) return true;
    ...
    try {
        ...
        if (!authState?.authenticated || !authState?.authorized || !authState?.user) {
            await redirectToLogin();
            return false;
        }

        syncSessionFromAuthState(authState.user);
        return true;
    } catch (e) {
        await redirectToLogin();
        return false;
...
const ensureMailServiceAvailable = async () => {
    if (mailServiceAvailable !== null) return mailServiceAvailable;
    if (!window.electronAPI?.invoke) {
        mailServiceAvailable = false;
        return mailServiceAvailable;
    }
    try {
        const res = await window.electronAPI.invoke('gmail-has-token');
        mailServiceAvailable = !!res?.connected;
    } catch (e) {
        mailServiceAvailable = false;
    }
    return mailServiceAvailable;
...
const sendEmailNotification = async (appointment, settings) => {
    ...
    const mailServiceReady = await ensureMailServiceAvailable();
    if (!mailServiceReady) {
        if (!lastEmailWarningShown) {
            toast.show('El servicio de correo del sistema no esta disponible.', 'warning');
...
document.addEventListener('DOMContentLoaded', async () => {
    const hasAccess = await ensureProtectedViewAccess();
    if (!hasAccess) return;
